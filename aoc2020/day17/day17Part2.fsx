...
// You should really use Array4D instead, it's faster by a billion
// Alternatively, don't use F# but something cool like C
// This took 90 seconds to run on my 8-core 11th gen i7-1165G7 @ 2.8 GHz
// which is about 90 seconds too long
...
let to4dmap xs : (Map<int*int*int*int,State>)  =
    let map = Map []
...
let neighbors (x,y,z,w) =
    seq { for i in x - 1 .. x + 1 do
          ...
          for k in z - 1 .. z + 1 do
          for l in w - 1 .. w + 1 do
          if (i,j,k,l) <> (x,y,z,w) then yield (i,j,k,l)
        }
...
let flipState minmax cubes =
    ...
    for x in range do
        for y in range do
            for z in range do
                for w in range do
                    let n = activeNeighbors (x,y,z,w) cubes                
                    if Map.containsKey (x,y,z,w) cubes then
                        if n = 2 || n = 3 then
                            map <- Map.add (x,y,z,w) Active map
                    else
                        if n = 3 then
                            map <- Map.add (x,y,z,w) Active map                    
    map
